 ...
 * @Date:   2022-03-24T18:45:46+08:00
 * @Last Modified by:   [FENG] <1161634940@qq.com>
 * @Last Modified time: 2022-03-26 09:49:26
 */
namespace feng;
...
class Calendar
{
    ...
    public function getSolarTerm($year, $month, $day)
    {
        ...
        $firstNode = $this->getTerm($year, ($month * 2 - 1)); // 返回当月第一个节气
        $secondNode = $this->getTerm($year, ($month * 2)); // 返回当月「节气」为几日开始

        if ($day < $firstNode) {
            $preMonth = $month - 1 == 0 ? 12 : $month - 1;
            $termIndex = $preMonth * 2 - 1;
        } elseif ($day == $firstNode || $day < $secondNode) {
            $termIndex = $month * 2 - 2;
        } else {
            $termIndex = $month * 2 - 1;
        }

        return $this->solarTerm[$termIndex] ?? '';
    }
     ...
     */
    public function getSeason($year, $month, $day)
    {
        ...
        $num = ceil(array_search($term, $termName)/6);
        return $this->season[$num] ?? '';
    }
    ...
    public function solar2lunar($year, $month, $day, $hour = null)
    {
        ...
        return [
            ...
            'term' => $term,
            'solar_term' => $this->getSolarTerm($year, $month, $day),
            'season' => $this->getSeason($year, $month, $day),
            'is_leap' => $isLeap,
        ];
    }
