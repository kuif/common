...
class Lunar
{
    ...
    /**
     * [convertSolarToLunar 将阳历转换为阴历]
     * @param  [type] $year  [公历-年]
     * @param  [type] $month [公历-月]
     * @param  [type] $date  [公历-日]
     * @return [type]        [description]
     */
    public function convertSolarToLunar($today)
    {
        list($year,$month,$day) = explode('-',$today);
        $yearData = $this->lunarInfo[$year-$this->MIN_YEAR];
        if($year==$this->MIN_YEAR&&$month<=2&&$day<=9) return array(1891,'正月','初一','辛卯',1,1,'兔');
        return $this->getLunarByBetween($year,$this->getDaysBetweenSolar($year,$month,$day,$yearData[1],$yearData[2]));
    }

    /**
     * [convertLunarToSolar 将阴历转换为阳历]
     * @param  [type] $year  [阴历-年]
     * @param  [type] $month [阴历-月，闰月处理：例如如果当年闰五月，那么第二个五月就传六月，相当于阴历有13个月，只是有的时候第13个月的天数为0]
     * @param  [type] $date  [阴历-日]
     * @return [type]        [description]
     */
    ...
    /**
     * [getSolarMonthDays 获取阳历月份的天数]
     * @param  [type] $year  [阳历-年]
     * @param  [type] $month [阳历-月]
     * @return [type]        [description]
     */
    ...
    /**
     * [getLunarMonthDays 获取阴历月份的天数]
     * @param  [type] $year  [阴历-年]
     * @param  [type] $month [阴历-月，从一月开始]
     * @return [type]        [description]
     */
    ...
    /**
     * [getLunarYearDays 获取农历每年的天数]
     * @param  [type] $year [农历年份]
     * @return [type]       [description]
     */
    ...
    /**
     * [getDaysBetweenLunar 计算阴历日期与正月初一相隔的天数]
     ...
     * @param  [type] $year   [阳历年]
     * @param  [type] $cmonth [阳历月]
     * @param  [type] $cdate  [阳历日]
     * @param  [type] $dmonth [另一个阳历月]
     * @param  [type] $ddate  [另一个阳历日]
     * @return [type]         [description]
     */
    ...
    /**
     * [getSolarTerm 节气通用算法]
     * @param  [type] $today [公历-(年-月-日)]
     * @return [type]        [description]
     */
    public function getSolarTerm($today)
    {
        list($_year,$month,$day) = explode('-',$today);
    ...
    /**
     * [getSeason 根据节气获取季节]
     * @param  string $value [公历-(年-月-日)]
     * @return [type]        [description]
     */
    public function getSeason($today)
    {
        $term = $this->getSolarTerm($today);
        $term_name = array(
            "立春","雨水","惊蛰","春分","清明","谷雨", // 春
            "立夏","小满","芒种","夏至","小暑","大暑", // 夏
            "立秋","处暑","白露","秋分","寒露","霜降", // 秋
        ...
        $season_name = array('春', '夏', '秋', '冬');
        $num = ceil(array_search($term['name1'], $term_name)/6);
        return $season_name[$num] ?? '';
    }

    /**
     * [getFestival 获取节日：特殊的节日只能修改此函数来计算]
     * @param  [type]  $today   [公历-(年-月-日)]
     * @param  boolean $nl_info [description]
     * @param  integer $config  [description]
     * @return [type]           [description]
     */
    public function getFestival($today, $nl_info = false,$config = 1)
    {
        ...
        $term = $this->getSolarTerm($today);
        if($term)array_push($festivals,$term);
